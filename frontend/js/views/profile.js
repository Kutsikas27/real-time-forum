...
export async function loadProfile() {
  ...
  const username = await fetch(`/user?userid=${userId}`)
    .then((d) => {
      return d.json();
    })
    .then((d) => {
  ...
  const userPosts = await fetch(
    `/profile/data?userid=${userId}&data=posts`
  ).then((d) => {
    return d.json();
  });
  const userComments = await fetch(
    `/profile/data?userid=${userId}&data=comments`
  ).then((d) => {
    return d.json();
  });
  const userPostLikes = await fetch(
    `/profile/data?userid=${userId}&data=postLikes`
  ).then((d) => {
    return d.json();
  });
  const userCommentLikes = await fetch(
    `/profile/data?userid=${userId}&data=commentLikes`
  ).then((d) => {
    return d.json();
  });
  let currentData,
  ...
  userPostsData = `<div class="created-posts">
            ...
            ${
              currentData === ""
                ? `<div class="no-rows">No rows to show</div>`
                : currentData
            }
        </div>`;
