...
import { getNicknameFromUserId, navigateTo, logout } from "../index.js";
...
export async function displayPost(id) {
  ...
  document.getElementById("forum-content").innerHTML =
    ...
    (postData.userlike === "like"
      ? 'class="liked"'
      : postData.userlike === "dislike"
      ? "disabled"
      : "") +
    `>👍</button>
                </aside>
...
export async function setLikesOnClick() {
  ...
  for (const btn of likebtns) {
    btn.addEventListener("click", async (e) => {
      ...
      let dislikeBtn = likeBtn.parentNode.parentNode
        .querySelector("#dislike-container")
      ...
      if (likeType === "like") {
        ...
        let dislikecontainer =
          e.target.parentNode.parentNode.querySelector("#dislike-container");
  ...
  for (const btn of dislikebtns) {
    btn.addEventListener("click", async (e) => {
      ...
      let likeBtn = dislikeBtn.parentNode.parentNode
        .querySelector("#like-container")
      ...
      if (likeType === "dislike") {
        ...
        dislikeCountElem.innerHTML = parseInt(dislikeCountElem.innerHTML) + 1;
        let likecontainer =
          e.target.parentNode.parentNode.querySelector("#like-container");
        let likeCountElem = likecontainer.querySelector("#like-count");
        if (likeType === "like") {
          likeCountElem.innerHTML = parseInt(likeCountElem.innerHTML) - 1;
        }
