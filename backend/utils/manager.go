...
func (m *Manager) setupEventHandlers() {
	...
	m.handlers[EventGetUsers] = GetUsersHandler
...
func (m *Manager) ServeWS(w http.ResponseWriter, r *http.Request) {
	...
	username := FindNicknameFromCookie(c.Value)
	userId := GetUserId(username)
	client := NewClient(conn, m, userId, username)
...
func (m *Manager) addClient(client *Client) {
	setUserStatus(client.user.UserId, "online")
...
func (m *Manager) removeClient(client *Client) {
	setUserStatus(client.user.UserId, "offline")
	SendStatusHandler(client)
	m.Lock()
	defer m.Unlock()
...
func setUserStatus(userId, status string) {
	_, err := Db.Exec(`UPDATE users SET staatus = ? WHERE userId = ?`, status, userId)
	if err != nil {
		log.Println(err)
	}
}
